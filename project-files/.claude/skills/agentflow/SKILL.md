...
**IMPORTANT: Launch the loop directly. Do NOT run `/af status` first.** The loop checks the board itself — a preliminary status check wastes time.
...
**When user asks for loop status** ("how's the loop?", "what's the progress?"):
1. Read `.agentflow/loop_status.txt` for quick loop state
2. Read `.agentflow/progress.txt` for completed work summary
3. Optionally read latest iteration file for details
...
1. Check current board state with `/af status` (via subagent)
2. Ask clarifying questions if multiple interpretations exist
...
**When starting the loop:** Don't run `/af status` first. Just launch the loop directly — it will check the board itself.
